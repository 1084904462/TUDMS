...
function isFilled(e){
	...
	if($e.attr("id") == "form-h-a"){
		var toolName = $("#form-h-a").val();
        $.ajax({
            url:'/UploadCheckServlet',
            type:'GET', //GET
            cache: false,
            ...
            success:function(result){
                if(result == "true"){
                	thisErrorHint.text("已有该工具");
                    thisErrorHint.css("display","inline-block");
				}
				else{
                    thisErrorHint.text("请输入工具名称");
                    thisErrorHint.css("display","none");
				}
            }
        })
...
function startUpload(){
    ...
    var firsterrorHint = $($(".errorHint")[0]);
	var firsterrorHintText = firsterrorHint.text();
	if(firsterrorHintText == "已有该工具"){
		firsterrorHint.css("display","inline-block");
        $('body,html').animate({
            scrollTop: firsterrorHint.offset().top - 100
        },500);
		return;
	}
